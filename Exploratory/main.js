...
function init() {
  ...
  yScale = d3
    .scaleLinear()
    .domain([0, d3.max(state.data, d => d.departures)])
...
function draw() {
  ...
  yScale.domain([0, d3.max(filteredData, d => d.departures)]);
        ...
        enter
          ...
          .on("mouseover", function(d) {      
            div.transition()      
                 .duration(200)      
                 .style("opacity", .9);      
                 div.text("Number of Departures: " + d.departures)
    ...
    .call(
      selection =>
        selection
          ...
          .attr("cy", d => yScale(d.departures)),// started from the bottom, now we're here
    );
